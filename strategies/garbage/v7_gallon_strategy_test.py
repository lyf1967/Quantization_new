...
class RSIHighFreqXAUUSD:
    def __init__(self, handler=None, dynamic_sl_enabled=True, dynamic_tp_enabled=True,
                 ...
                 stop_loss_cooling=10, # 默认值45*60
                 take_profit_cooling=10, # 默认值45*60
                 max_stop_loss=-0.3, # 默认值-5，未使用（改为加仓）
                 min_take_profit=0.1,  # 默认值1.5，未使用（改为多级）
                 ...
                 strict_buy_rsi=49,  # 严格买入RSI，默认30
                 strict_sell_rsi=50,  # 严格卖出RSI，默认70
                 addon_loss_thresholds=[-0.2, -0.3],  # 加仓亏损阈值（0.01手美元），第一级-10，第二级-60  [-10, -60]
                 addon_tp_mins=[0.05, 0.1, 0.0],  # 各级最小止盈（0.01手美元），初始1.5，第一加仓后3.0，第二后0.0  [1.5, 3.0, 0.0]
                 max_positions = 1,
