...
class RSIHighFreqXAUUSD:
    def __init__(self, handler=None, dynamic_sl_enabled=True, dynamic_tp_enabled=True,
                 ...
                 addon_loss_thresholds=[-0.1, -400],  # 加仓亏损阈值（0.01手美元），第一级-30，第二级-400  [-30, -400]
                 add_times_list = [2, 2], # 加仓倍数：第一次加仓2倍，第二次加仓2倍
                 addon_tp_mins=[0.1, 2*0.1, 9*0.5],  # 各级最小止盈（0.01手美元），初始2，第一加仓后3.0*1，第二后9*0.5  [2, 3.0*1, 9*0.5]
                 max_positions = 1,
        ...
        self.add_times_list = add_times_list
        # 跟踪当前持仓组
    ...
    def start_dynamic_sl_monitor(self, symbol, current_initial_volume=0.01):
        """启动动态止损和止盈监控线程"""
        if not self.handler:
        ...
        def monitor_thread():
            self.monitor_dynamic_sl(symbol, current_initial_volume=current_initial_volume)
...
def get_signal(data, symbol, handler=None, dynamic_sl_enabled=False, dynamic_tp_enabled=False, current_initial_volume=0.01):
    """顶级的get_signal函数，供MT5Handler调用"""
    strategy = RSIHighFreqXAUUSD(handler=handler, dynamic_sl_enabled=dynamic_sl_enabled, dynamic_tp_enabled=dynamic_tp_enabled, current_initial_volume=current_initial_volume)
